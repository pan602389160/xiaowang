...
process_and_exit:
	...
	memset(&mode, 0, sizeof(wifi_ctl_msg_t));
	mode.force = true;
	strcpy(mode.name, "from_app");
	if (!strcmp("AP", cmd)) {
		strcpy(mode.param.switch_ap.ssid, ssid);
		strcpy(mode.param.switch_ap.psk, passwd);
		mode.cmd = SW_AP;
	}
	else if (!strcmp("STA", cmd)){
		strcpy(mode.param.switch_sta.ssid, ssid);
		strcpy(mode.param.switch_sta.psk, passwd);
		mode.cmd = SW_STA;
	}
