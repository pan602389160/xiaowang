                ...
                int width, int height, int bpp, Uint32 flags)
{
	...
	fprintf(stdout,"*****xhhu******in FB_SetVideoMode:508*************\n");
...
#endif
    {
		...
		fprintf(stdout,"*****xhhu******in FB_SetVideoMode:520*************\n");
            return(NULL);
        }
#endif
	...
	fprintf(stdout,"*****xhhu******in FB_SetVideoMode:527*************\n");
    }

    /* Set the video mode and get the final screen format */
    if (ioctl(console_fd, FBIOGET_VSCREENINFO, &vinfo) < 0 ) {
	fprintf(stderr,"*****xhhu******in FB_SetVideoMode:ioctl error*************\n");
        GAL_SetError("@@@@0_0NEWGAL>FBCON: Couldn't get console screen info\n");
        return(NULL);
    }
	fprintf(stdout,"*****xhhu******in FB_SetVideoMode:536*************\n");
#ifdef FBCON_DEBUG
    ...
    if ( ((vinfo.xres != width) || (vinfo.yres != height) ||
        ...
        if ( ioctl(console_fd, FBIOPUT_VSCREENINFO, &vinfo) < 0 ) {
            vinfo.yres_virtual = height;
            if ( ioctl(console_fd, FBIOPUT_VSCREENINFO, &vinfo) < 0 ) {
		fprintf(stdout,"*****xhhu******in FB_SetVideoMode:573*************,height=%d\n",height);
                GAL_SetError("@_@NEWGAL>FBCON: Couldn't set console screen info\n");
                return(NULL);
            }
        }
    } else {
