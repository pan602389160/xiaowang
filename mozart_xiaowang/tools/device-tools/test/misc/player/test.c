...
int main(int argc, char **argv)
{
	...
	int c = -1;
	while (1) {
		c = getopt(argc, argv, "c:u:f:h");
		if (c < 0)
			break;
		switch (c) {
			case 'c':
				ch_num = strtol(optarg, NULL, 0);
				break;
			case 'u':
				strcpy(def_uuid, optarg);
				break;
			case 'f':
				file = optarg;
				break;
	...
	handler = mozart_player_ch_handler_get(def_uuid, ch_num, callback, NULL);
	if (!handler) {
