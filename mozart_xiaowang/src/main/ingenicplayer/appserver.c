...
{	//printf("=======appserver_cmd_callback===========scope = %s, command = %s,data = %s\n",scope,command,data);
	...
	if (strcmp(appserver_cmd[INGENICPLAYER_SCOPE], scope) == 0) {
		ret = mozart_ingenicplayer_response_cmd(command, data, owner);
		return ret;
	} else if (strcmp(appserver_cmd[ALARM_SCOPE], scope) == 0) {
#if (SUPPORT_ALARM == 1)
		ret = mozart_alarm_response_cmd(command, data, owner);
#else
		ret = mozart_appserver_response("alarm_unsupport", NULL, owner);
#endif
		return ret;
	} else if (strcmp(appserver_cmd[DEVICE_SCOPE], scope) == 0) {
		ret = mozart_device_response_cmd(command, data, owner);
		return ret;
	}else if(strcmp("usercommand", command) == 0){//usercommand
