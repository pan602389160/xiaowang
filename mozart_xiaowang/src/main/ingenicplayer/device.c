...
{	printf("===============get_detail_device_info===========set_battery : %d\n",set_battery);
	...
	char macaddr_x[] = "255.255.255.255";
	FILE *fp = NULL;
	json_object *object = NULL;
	json_object *tmp = NULL;
	...
	if(fd > 0){
		...
		strncpy(macaddr, id_buf,strlen(id_buf));
		macaddr[strlen(id_buf)] = '\0';
		close(fd);
	}else{
		sprintf(macaddr,"%s","tmp_ID");
	}

	//get_mac_addr("wlan0", macaddr_x, "");
	//sprintf(macaddr,"NS-%s",&macaddr_x[6]);
	//sprintf(macaddr,"%s","NS-0E9E91");
	...
	/* device electricify */
	if ((tmp = json_object_new_int(set_battery)) == NULL)
		goto device_info_err;
