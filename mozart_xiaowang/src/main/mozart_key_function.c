...
{	
	...
	_mywav_header buff;
	int res=0;
	audioP = fopen(audioName,"rb");//Res=fopen(&audioP,audioName,FA_READ);
	if(audioP != NULL){
		fread(&buff,1,sizeof(_mywav_header),audioP);//Res=f_read(&audioP,(void *)&buff,sizeof(_mywav_header),&readsize);
		dindx=0;
...
void *record_func(void *args)
{
	usleep(2500*1000); //等待语音 “进入网络配置模式”   播放完
	acoustic_netconfig_start();
	printf("-------------------------------------------------------------->FFT\n");
	readWave("/usr/data/asr_dmic_16.wav");
	return NULL;
}
#endif
void mozart_config_wifi(void)
{
	printf("-------mozart_config_wifi-------\n");

	#if 0
	pthread_t record_pthread;
	if (pthread_create(&record_pthread, NULL, record_func, NULL) == -1) {
		printf("Create record_pthread pthread failed: %s.\n", strerror(errno));
		return NULL;
	}
	pthread_detach(record_pthread);
...
extern bool music_playing_configing;
extern pthread_t music_playing_pthread;

extern bool blink_configing ;
extern pthread_t blink_pthread;

extern bool wificonfiging_configing;
extern pthread_t wificonfiging_pthread;
extern int fd;
...
{	printf("bluetooth_mode_pthread=============================================>>>>>>>>>the tid = %d\n",pthread_self());
...
extern int need_resume_after_asr;
void mozart_play_pause(void)
{
	...
	switch (domain) {
	case UNKNOWN_DOMAIN:
		printf("snd_source = %d,system is in idle mode in %s:%s:%d.\n",snd_source,__FILE__, __func__, __LINE__);
		#if (SUPPORT_LOCALPLAYER == 1)
		#if (SUPPORT_BT == BT_BCM)
...
{	
	usleep(100*1000);//按键太快的话模式切换时间
	printf("------mozart_snd_source_switch-------- mozart_vr_get_status = %d\n",mozart_vr_get_status());
	memory_domain domain;
	int ret = share_mem_get_active_domain(&domain);
	if(domain == UNKNOWN_DOMAIN){
		printf("[snd_source = %d,%s, %d]: Not support domain is active\n",snd_source, __func__, __LINE__);
		//开机none（snd_source = -1），不允许切换，会造成播放模式和其他模式混在一起
		//蓝牙和本地卡没内容的话可以切换
		if(snd_source != SND_SRC_BT_AVK&&snd_source != SND_SRC_LOCALPLAY&&snd_source != SND_SRC_CLOUD){
			printf("cant mozart_snd_source_switch\n");
			return;
		}
	}
	if (mozart_vr_get_status() == VR_ASR){  //唤醒状态无法切换
		//printf("VR_ASR VR_ASR VR_ASR\n");
		//return;
		need_resume_after_asr = -1;
		mozart_stop_tone_sync();
		mozart_vr_asr_break();
	...
	mozart_module_stop();//\D4\DDͣ\B5\B1ǰģʽ
	// find the next snd source.
	while (1) {
		source = (source + 1) % SND_SRC_MAX;
		printf("11 source = %d, wifi_mode = %d\n",source,get_wifi_mode().wifi_mode);	
#ifdef SUPPORT_SONG_SUPPLYER // 0
		if (source == SND_SRC_CLOUD && get_wifi_mode().wifi_mode != STA){
			continue;
		}
#endif

#if (SUPPORT_LOCALPLAYER == 1) // 2
...
#if (SUPPORT_INGENICPLAYER == 1)// 3
		if (source == SND_SRC_INGENICPLAYER){
			continue;
		}
#endif

		if (source == SND_SRC_LINEIN && !mozart_linein_is_in()){// 4
			continue;
		}
		break;
	}
...
//		return ;
	printf("switching to %s mode.  source = %d\n\n", snd_source_str[source],source);
	
	switch (source) {

	case SND_SRC_CLOUD:
		printf("--->SND_SRC_CLOUD\n");		
		break;
	case SND_SRC_BT_AVK:
		printf("--->SND_SRC_BT_AVK\n");
